...
$cnd = "";
$cnd = "WHERE PAR_NOTUSED NOT LIKE '+'";
...
$(document).ready(function(){
	$("tr#tbitem").click(function() {        // function_tr
		...
		window.opener.document.theForm.description_cost.focus();
		self.close(); 		
	});
});
</script> 
...
<body> 
<div class="filters">
<form action="<?php echo $_SERVER['PHP_SELF'].'?obj='.$tableName.'&budget_amount_cost='.$budget_amount_cost.'&classification_cost='.$classification_cost.'&costCommodity='.$costCommodity.'&CMD_CODE='.$CMD_CODE
.'&january='.$january.'&february='.$february.'&march='.$march.'&april='.$april.'&may='.$may.'&june='.$june.'&july='.$july
.'&august='.$august.'&september='.$september.'&october='.$october.'&november='.$november.'&december='.$december.'&id_cost='.$id_cost; ?>" method="post" name="theForm" enctype="multipart/form-data">
