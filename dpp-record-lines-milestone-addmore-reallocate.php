			...
			$result2 = 0;
				...
				if (@$_POST['milestone']){
					for ( $i=0;$i<count($_POST['milestone']);$i++) {
						...
						$data2 = array("id"=>$id,"milestone"=>$milestone,"Jan"=>$Jan,"Feb"=>$Feb,"Mar"=>$Mar,"Apr"=>$Apr,"May"=>$May,"Jun"=>$Jun,"Jul"=>$Jul,"Aug"=>$Aug,"Sep"=>$Sep,"Oct"=>$Oct,"Nov"=>$Nov,"Dec"=>$Dec,"budget_amount"=>$budget_amount,"available"=>0,"createdAt"=>$today2,"updatedAt"=>$today2);	
						$result2 = $crudapp->insertAddmoreMilestone2($conn,$data2,$table2);
					}	
				}

				header("Location:dpp-budget-movement-project.php?login=$user&ORG_CODE=$org_code&year=$year&from_id=$from_id&from_val=$from_val&to_id=$result2&to_val=$milestone&amount=$budget_amount&prj_code=$project_id&prj_desc=$project_description&movementType=reallocation");
